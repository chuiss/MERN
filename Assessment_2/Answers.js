...
/*
-Statelessness: Statelessness refers to the property of a system where each request from a client to a server is treated as an independent 
transaction. 
The server does not retain any information about the client's previous requests. This means that each request must contain all the
 information needed for the server to fulfill it.
...
-spa: SPA (Single Page Application) is a web application that interacts with the user by dynamically rewriting the current page,
 rather than loading entire new pages from the server.
This approach allows for a more fluid and responsive user experience, as only the necessary content is updated without the need for 
full page reloads.

-classical applications: Classical applications are traditional web applications that follow a multi-page architecture.
They typically involve loading a new HTML page from the server for each user interaction, which can lead to slower response 
times and a less seamless user experience compared to SPAs.
*/
...
/*
React uses a virtual DOM to optimize the rendering process. When a component's state or props change,
React creates a new virtual DOM representation of the component. 
...
When a state update occurs, React schedules a re-render of the component and its children, 
allowing it to efficiently update the virtual DOM and subsequently the actual DOM.  
*/
...
This process identifies the differences between the two trees, 
allowing React to determine the minimal set of changes needed to update the actual DOM.
*/